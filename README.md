...
만들어주는 Chrome 확장프로그램(Manifest V3)입니다.
...
YouTube 시청 페이지(`youtube.com/watch?v=...`)를 열면 좋아요/공유 버튼 근처에
**"⚡ 초고밀도 분석 노트 생성"** 버튼이 나타납니다. 버튼을 누르면:

1. 오늘의 일일 사용 한도를 초과하지 않았는지 먼저 확인합니다.
2. Gemini API에 `processing: "agentic"` 옵션으로 영상 URL을 직접 전달해 분석을 요청합니다.
   (영상을 직접 업로드하지 않고 URL만 전달합니다.)
3. 분석이 끝나면 버튼에 완료/실패 여부가 잠시 표시되고, 결과는 자동으로 히스토리에 저장됩니다.
4. 확장프로그램 팝업을 열면 **"현재 분석"** 탭에서 완성된 리포트를 바로 확인할 수 있습니다.
