...
## v1.2.0 제출 메모 (소제목 → 영상 장면 이동, 공유 출처 표기·리뷰 요청)
...
- **권한 변경 없음.** 기존 `tabs` 권한(열린 YouTube 탭 찾기)과 YouTube 콘텐츠 스크립트(재생 위치
  옮기기)만 씁니다. 리뷰 페이지는 사용자가 버튼을 누를 때 새 탭으로 열 뿐입니다.
- **자세한 설명 갱신.** 한국어·영어 "리포트 구성"에 ▶ 시각 이동 한 줄이 추가됐고, 복사/다운로드
  줄에 출처 표기 설명이 붙었습니다.
- **개인정보처리방침 갱신됨(자동 반영).** 3항에 "▶ 버튼을 누르면 그 영상 탭의 재생 위치만
  옮긴다"는 문장이, 1항 표에 "리뷰 요청 상태(로컬 저장)" 행이 추가됐습니다.
